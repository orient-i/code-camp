...
pnpm 是由 Zoltan Kochan 在 2017 年 6 月发布的新一代包管理器。它具有安装速度快、节省磁盘空间、安全性高等优点，它的出现进一步解决了 npm 和 yarn 存在的问题。

#### 速度块

下面是 pnpm 官网提供的 benchmark 数据对比：
...
<img src="../assets/pnpm-benchmark.svg" />

可以看到，在多数场景下，pnpm 的包安装速度都是明显优于 npm/yarn 的。

#### 节省磁盘空间

pnpm 内部使用基于内容寻址的文件系统来存储磁盘上的所有文件，这种设计的出色之处在于：
...
尽管 lock 文件帮助 npm 和 yarn 解决了不确定性的问题，但扁平化的 node_modules 结构所带来的幽灵依赖依然是个还未解决的安全隐患。接下来，我们看看 pnpm 下的 node_modules 目录结构是什么样的。

还是使用前面的例子，在项目根目录下执行 `pnpm install` 之后，可以看到项目的 node_modules 目录结构如下图所示：

<img src="../assets/pnpm-nodemodules.png" />

从上面的图中不难发现下面这两个点：

1. node_modules 目录下不存在被提升的依赖，所有依赖都是项目的直接依赖，完美地和项目的依赖定义对应上，直接杜绝了幽灵依赖产生的前置条件。
