...
- 每个项目的 `node_modules` 下都有一个 `.pnpm` 目录，项目的所有依赖(包括子依赖)以 `package-name@version` 的格式完全平铺在这个目录中。而 `package-name@version` 这个目录下以完全打平的结构管理项目的所有依赖(包括子依赖)，这些依赖则以硬链接的方式指向 .pnpm-store 里的真实文件内容地址。
- 每个项目的 `node_modules` 以类似于 npm v2 的嵌套结构管理项目的所有直接依赖，这不仅完美契合了 node 查找依赖的规则，还杜绝了幽灵依赖产生的条件。此外，这些直接依赖并不是真实文件，而是 .pnpm 下依赖包的软链接。

也就是说，每个依赖包的查找需要经过三层结构：`node_modules/package-name` -> `node_modules/.pnpm/package-name@version/node_modules/package-name` -> `~/.pnpm-store/v3/files/xxx`

#### 示例

这里继续使用前面的例子，来看看在项目根目录下执行 `pnpm install` 之后，项目的 node_modules 目录结构是否如前面所说那样。
...
这是因为 mod-a 并不是真实的源文件，它其实是个软链接。文件夹右侧的箭头直观地表明了它的身份，我们也可以通过 `readlink` 命令来识别出这一点：
...
从命令结果可以看到，mod-a 链接的真实文件地址是在 .pnpm 文件夹下，展开 .pnpm 之后，果然发现了 mod-a：

<img src="../assets/pnpm-nodemodules2.png" />

从图中可以看到项目的所有依赖(不管是直接依赖还是子依赖)都以 `<package-name>@version` 的格式直接平铺在了 .pnpm 里面，非常直观。此外，将项目的直接依赖本体和其对应的子依赖放在同一个 node_modules 目录下，完美地兼容了原生 node 的依赖查找方式。

#### 优势全解

下面是官网关于 pnpm 的依赖管理机制的说明图，诠释了 pnpm 的三大优势：
